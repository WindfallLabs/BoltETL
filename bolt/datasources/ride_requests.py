...
class RideRequests(Datasource):
    ...
    def transform(self):
        ...
        df.loc[df["Requested Pickup Time"].isna(), "Requested Pickup Time"] = df[
            "Original Planned Pickup Time"
        ]
        self.logger.info(
            "Set Null 'Requested Pickup Time' values to 'Original Planned Pickup Time'"
        ...
        df["Cancel Time Diff (hours)"] = df["Cancel Time Diff (hours)"].astype(
            types.pyarrow_float32
        )
        ...
        df.loc[cancelled_not_excused, "Cancel Time Diff (hours)"] = (
            (df["Requested Pickup Time"] - df["Cancellation Time"]).dt.total_seconds()
            / 3600
        ).round(3)
        ...
        df.loc[
            ...
            & (df["Cancel Time Diff (hours)"] < LATE_PARA_CANCELLATION_HRS),
            "Late Cancellation",
        ] = "Y"
        self.logger.info(
            "Calculated 'Late Cancellation' values for 'Paratransit' service"
        )
        ...
        df.loc[
            (
                ...
                & (
                    df["Cancellation Time"]
                    >= (
                        (
                            df["Original Planned Pickup Time"] - dt.timedelta(days=1)
                        ).dt.normalize()
                        + dt.timedelta(hours=16, minutes=30)
                    )
                )
            ),
            "Late Cancellation",
        ...
        self.logger.info(
            "Calculated 'Late Cancellation' values for 'Microtransit' service"
        )
