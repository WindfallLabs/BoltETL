...
def get_holiday_service(year: int, drop_observed=False) -> pd.DataFrame:
    ...
    service = pd.DataFrame(
        [(k, v) for k, v in mutd_holidays.items()],
        columns=["Holiday", "Service"]
        ).set_index("Holiday")
    ...
    holiday_df.sort_values(
        "Date",
        inplace=True,
        key=lambda x: x.dt.strftime("%Y%m%d")
        )
    ...
    holiday_df.loc[holiday_df["Service"] == "Normal", "Service"] = (
        holiday_df.loc[
            holiday_df["Service"] == "Normal",
            "Day"].apply(_match_day)
    )
    return holiday_df
...
def get_month_days(year: int, month: int) -> pd.DataFrame:
    ...
    df = pd.DataFrame(
        [(dt.datetime(year, month, day), day_name) for day, day_name in cal_zip
         if day > 0],
...
def get_full_calendar(year: int, month: int, drop_observed=False) -> pd.DataFrame:
    ...
    full_cal_df: pd.DataFrame = month_days.join(holiday_df.set_index("Date"), rsuffix="_")
    ...
    full_cal_df["ServiceType"] = full_cal_df.apply(
        lambda x: x["DayName"] if x["DayName"] in ["Saturday", "Sunday"] and x["Service"] is nan else x["Service"],
        axis=1)
...
def get_service_days(year: int, month: int) -> pd.DataFrame:
    ...
    full_cal_df: pd.DataFrame = get_full_calendar(year, month, drop_observed=True)  # Static True value
    # Group the full calendar dataframe by ServiceType and count the days for each type
    df: pd.DataFrame = (
        full_cal_df.groupby("ServiceType")[["DayName", "Holiday"]].count()
